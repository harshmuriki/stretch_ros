#!/usr/bin/env python3
"""
This script defines a ROS node that subscribes to action commands and joint states, and moves a robotic arm relative to its current position based on received commands. The node uses an action client to send trajectory goals to a trajectory server.
Classes:
    StowCommand: A class that inherits from hm.HelloNode and handles the movement of the robotic arm based on received action commands.
Methods:
    __init__(self): Initializes the StowCommand class, sets up subscribers, and waits for the trajectory action server to be available.
    joint_state_callback(self, msg): Callback function to update the current joint positions based on received JointState messages.
    move_arm_relative(self, lift_offset, wrist_offset, gripper_offset): Moves the arm relative to the current joint positions by specified offsets.
    action_callback(self, msg): Callback function to handle received action commands and move the arm accordingly.
Main function:
    Initializes the ROS node, creates an instance of StowCommand, and keeps the node running.
"""
...
class StowCommand(hm.HelloNode):
    ...
    def move_arm_relative(self, lift_offset, wrist_offset, gripper_offset):
        ...
        current_gripper = self.current_joint_positions.get(
        ...
        move_point.time_from_start = rospy.Duration(
            1.0)  # Duration for the movement
        ...
        trajectory_goal.trajectory.joint_names = [
            'joint_lift', 'wrist_extension', 'joint_gripper_finger_left']
