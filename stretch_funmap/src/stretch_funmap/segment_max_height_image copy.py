...
def find_object_to_grasp(height_image, display_on=False, object_bbox=None):
    # cv2.imwrite("/home/hello-robot/Documents/lmnavgrasp/image6.jpg", height_image.rgb_image)
    # cv2.imwrite("/home/hello-robot/Documents/lmnavgrasp/image7.jpg", height_image.image)
    cv2.imwrite("/home/hello-robot/Documents/lmnavgrasp/image8.jpg", height_image.rgb_image)
    ...
    # label_image = np.zeros((w, h), dtype=int)

    # ! get data from SAM
    # Send the max_h_image to /sam_model topic
    # receive list of [x, y] coordinates of the detected object from /detected_coordinates topic

    # save the image
    cv2.imwrite("/home/hello-robot/Documents/lmnavgrasp/image7.jpg", height_image.rgb_image)
    # cv2.imwrite("/home/hello-robot/Documents/lmnavgrasp/image5.jpg", obstacles_on_surface)
    ...
    for i in range(2):
        ...
        publish_image(height_image.rgb_image)
        rospy.sleep(1)

    rospy.Subscriber('/detected_coordinates', UInt8MultiArray, detected_coordinates_callback)
    rospy.sleep(2)  # Sleep for a second to allow for message delivery

    rate = rospy.Rate(10)  # 10 Hz
    cnt = 0
    while not callback_received and not rospy.is_shutdown():
        print("waiting")
        cnt += 1
        rate.sleep()
    ...
    if callback_received:
        ...
        print("Callback received, proceeding with the rest of the code")
        ...
        print("sam_clip_opt")
        print(sam_clip_data)
        print(type(sam_clip_data[0]))
        print(30*'*')

    else:
        # use the current image
        print("No image received so using the current live image")
        print("Shutdown requested before callback was received")
    ...
    colored_image = np.zeros((label_image.shape[0], label_image.shape[1], 3), dtype=int)
    ...
    if len(region_properties) > 0:
        ...
        object_height_selector = object_selector #obstacles_on_surface_selector &  #! Should set the & when i get the actual coordinates with SAM
        ...
        print("grasp loc of ellipse:", grasp_location_xy_pix, diff_m, object_ellipse)
...
def segment(image, m_per_unit, zero_height_m, segmentation_scale=0.1, verbose=False):
    ...
    # getting all the non-zero pixels
    ...
    # basic segmentation done here => now using SAM
...
def test_segment(image, mm_per_unit, zero_height_unit, segmentation_scale=0.1, verbose=True, visualize=True):
    ...
    for segment_id in segment_info:
        height_unit = segment_info[segment_id]['height_unit']
        height_m = mm_per_unit * (height_unit - zero_height_unit)
        segment_info[segment_id]['height_m'] = height_m
